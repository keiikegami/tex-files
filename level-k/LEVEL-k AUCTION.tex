...
\section{Models}
	...
	\subsection{Nonequilibrium Level-k Models}
		...
		\begin{frame}\frametitle{Random L2 , Truthful L1 and Truthful L2}
			...
			\item For Random L2 in this case, the highest bid among the other players conveys the information about the value of the object, then Random L2 adjusts its own estimated value according to the information. This means that it uses $\upsilon(x, y)$ rather than $r(x)$.
		...
		\begin{frame}[shrink]\frametitle{Strategic Substitutability}
			\begin{itemize}
			\item "Value adjustment (using $\upsilon(x,x)$) tends to make bidders' bids strategic substitutes"  means that a player underbids when the others overbid and vice versa.
			\item The player who thinks the others overbid regards their private signals as smaller than ones estimated when the bidding is equilibrium.
			\item He adjusts the value of the object following his own estimated private signals of others. Then the bidding is less than equilibrium due to the small private signals.
			\item Thus He underbids when he thinks that the others overbid.
			\end{itemize}
		\end{frame}
...
\section{Comparing}
	...
	\subsection{Econometrical Comparing}
		...
		\begin{frame}\frametitle{Verification}
			\begin{itemize}
			\item The choice probability function (26) generates the highest value when $c_k^g(x)$ is inputed.
			\item Since $S_k^g(c | x)$ is quasi concave, the higher the probability is, the $c_{it}^g$is closer to the optimal bidding specific to type and game. Thus the likelihood function works.
			\end{itemize}
		\end{frame}
		
		\begin{frame}\frametitle{Note}
			\begin{itemize}
			\item Truthful L0 is omitted because truthful bidding is very rare in first price auction and truthful bidding can not be optimal under any belief.
			\item ($\sim X$) means the bidding strategy of the type is not separated from one of type $X$, so the estimation of the parameters is done about the mixed type. This notation is used in the following tables.
			\item BIC is used because there are a lot of parameters in subject specific precisions specification.
			\end{itemize}
		\end{frame}
		
		\begin{frame}\frametitle{Table3a}
			\begin{itemize}
			\item KL first price example.
			\item Likelihood ratio test reject the constant or type specific precisions.
			\item Cursed equilibrium has a modest advantage over level-k plus equilibrium model in both likelihood and BIC.
			\item This is due to the tractability of cursed equilibrium model about the types with intermediate cursedness.
			\end{itemize}
		\end{frame}
		
		\begin{frame}\frametitle{Table3c}
			\begin{itemize}
			\item Ak second price example.
			\item The estimated frequency of Random L0 is 0\%, which implies that Random L0 exists in the minds of Random L1.
			\item Level-k plus equilibrium model has substantial advantages in likelihood and BIC over cursed equilibrium model.
			\end{itemize}
		\end{frame}
		
		\begin{frame}\frametitle{Table3d}
			\begin{itemize}
			\item GHP first price example.
			\item Level-k plus equilibrium model has substantial advantages in likelihood and BIC over cursed equilibrium model.
			\item The frequencies of each type in level-k model in Table $a,c,d$ are very stable.
			\end{itemize}
		\end{frame}
		
		\begin{frame}\frametitle{Table3b}
			\begin{itemize}
			\item KL second price example.
			\item Level-k plus equilibrium model has substantial advantages in likelihood and BIC over cursed equilibrium model.
			\item The estimated frequencies of each type are very different from those of the other examples. This is mainly because the frequency of Truthful L2 is estimated highly.
			\item This wrong estimation occurs since, through estimation procedure, the subjects bidding above equilibrium who just make a mistake about the optimal bidding strategy in second price auction are captured as Truthful L2, which thinks the other players underbid relative to the equilibrium.
			\end{itemize}
		\end{frame}
...
\section{Conclusion}
	\begin{frame}\frametitle{Conclusion}
		\begin{itemize}
		\item Level-k model has the potential to improve the explanation about the non optimal biddings in the variable types of auctions which were previously studied.
		\item And the estimation of the parameters coincides with the results of previous studies.
		\end{itemize}
	\end{frame}
