...
2番目のケースで$B \in \sigma(\mathcal{C_B})$となる$\mathcal{C}$の可算部分集合$\mathcal{C_B}$が存在することが示され田とする。この時、$\sigma$-fieldが要素の補集合について閉じていることを考えると、1番目のケースについては1サイクル前の終了時に得られている$\mathcal{C}$の要素を$B$として引いた時に構成される$\mathcal{C_B}$に含まれていることから、自動的に示されることになる。従って、2番目のケースについてのみ考えれば良い。
