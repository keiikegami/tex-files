...
\section{Analysis}
	\subsection{No quality gap setting}
	\par
	...
	\begin{center}
                ...
                \end{tabular}
        \end{center}
        \caption{no quality gap setting}
  	\end{table}
	\par
	...
	\subsection{One side quality gap setting}
	...
	\begin{table}[h]
		\begin{center}
                ...
                \end{tabular}
                \end{center}
                \caption{one side quality gap setting}
          \end{table}
	...
	\par
	For simplicity let $\theta > 0$. I assume that $x_h > x_l > y$. This is the key assumption. Now $H$ strictly dominates $L$ for $a$ by the assumption. Thus if $b$'s payoff of choosing $L$ is bigger than one of $H$, $[H, L]$ can be achieved. The condition is as follows.
	\begin{align*}
	...
	\end{align*}
	The last line is due to the $x_h > x_l > y$. And this assumption also allows $\frac{x_h - x_l}{2} \frac{1}{x_h + x_l - 2y}$ to be less than $\frac{1}{2}$.
	\par
	In this condition, the probability of success is the probability of "$H$ chooses $a$ and $L$ chooses $b$". Now I get the below.
	\begin{align*}
	...
	\end{align*}
	The above shows that the lower bound of the probability of success is the decreasing function of $\frac{x_l - y}{x_h - y}$. And this is the increasing function of $x_h$ with fixed $x_l$. Then in this setting "Sekigae" should not be carried out. And the bigger gap among one group exists, the more likely all members get partners.
	\par
	...
	\subsection{Two side quality gap setting}
	In this case, all the players know which of the other group members is high quality or low quality and know the same among their own group. This is a highly strategic situation. The payoffs are summarized in Table5, where $x_h$ is the payoff for matching with a high quality person and $x_l$ is one with a low quality person ($x_h > x_l> y$). The notations in the previous settings are used in Table5.
	...
	\begin{center}
                ...
                \end{tabular}
        \end{center}
        \caption{two side quality gap setting}
  	\end{table}

	\par
	First I consider "Sekigae" case and assume that all people in this model think "High quality persons match high quality one". I introduce this assumption as an idea accepted in common. This assumption make the payoff matrix very simple as in table 6.
	...
	\par
	Note $x_h > x_l > y$.
	This matrix is the left upper part of Table5. And in this situation choosing the low quality dominates the other choice for both of them. Then $((h, l), [H, L])$ is always achieved as a whole. This means the probability of two matching is 1. Furthermore I get the same result under "Low quality persons can not match with High quality persons, so should choose Low quality persons" assumption. In this situation The payoff matrix can be simplified like the previous one and choosing high quality dominates choosing low quality for the two high quality participants. Then $((h, l), [H, L])$ is always realized.
	\par
	In no "Sekigae" case, I write the payoff gain from the longer time of talking as $\tau$, but it ends up to the same result under the common idea assumptions provided however $\tau$ is large. Then I relieve the assumption and consider the general case equilibriums. But I only consider the case with "Sekigae" due to the words restriction.
	\par
	...
	\end{align}
	When he chooses $l$, the expected payoff is as follows.
	\begin{align}
	&bcdx_l + bc(1-d)y + b(1-c)dx_l + b(1-c)(1-d)y \nonumber \\&+ (1-b)cdx_l + (1-b)c(1-d)y + (1-b)(1-c)dx_l + (1-b)(1-c)(1-d)y
	\end{align}
	these can be calculated from Table5. Now the border between the two choices is 
	\begin{align}
	...
	\end{align}
	The same logic can be used for $L$. Thus I omit the detail and get the below border.
	\begin{align}
	...
	\begin{align*}
		...
		a = 0, b = 1 & \left( 0 \leq c < \frac{x_l - y}{x_h - y}d \right)
            	\end{cases}
	\end{align*}
	And the same logic is applied to compute the best response function about $c$ and $d$.
	\begin{align*}
            	\begin{cases}
		...
		c = 0, d = 1 & \left( 0 \leq a < \frac{x_l - y}{x_h - y}b \right)
            	\end{cases}
	\end{align*}
	The mixed strategy Nash equilibriums are the the pairs of the probabilities which respond mutually best. I find four mixed strategy Nash equilibriums by studying the above best response functions.
	\begin{align}
	&(a = 1, b = 0, c = 1, d = 0)\\
	&(a = 1, b = 1, c =1, d = 1)\\
	...
	&(a = 0, b = 1, c = 0, d = 1)
	\end{align}
	If each of these four equilibriums are realized by the same probability ($\frac{1}{4}$ for each), the probability of two pair matching is $P = \frac{1}{4} + \frac{1}{4} + \left( 1 - \frac{x_l - y}{x_h - y} \right)^2$, which is a increasing function about $x_h$ with fixed $x_l$. And $P \to \frac{3}{4}$ as $x_h \to \infty$.

\section{Conclusion}
\par
In this paper I study the strategic characteristics of Japanese singles party, particularly considering "no quality gap setting", "one side quality gap setting" and "two side quality gap setting". These settings are three from the cases which are classified by the recognition of quality gap in the participants own group and by the knowledge in each gender group about the quality gap in the other group. 
\par
